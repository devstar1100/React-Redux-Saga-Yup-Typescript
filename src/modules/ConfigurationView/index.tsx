...
import {
  ...
  duplicateConfigurationFileServer,
...
const ConfigurationFilesList = () => {
  const dispatch = useDispatch();
  const navigate = useNavigate();

  const rowsServer = useSelector(getConfigurationFiles);
  const isLoading = useSelector(getConfigurationFilesLoading);
  ...
  const filterRows = (el: Row) => {
    if (searchValue && !searchMatch(el.cells[1].text as string, searchValue)) return false;
  ...
  const handleActionCustomViews = (action: (props: Required<GetConfigurationFilesServerProps>) => any) => () => {
    selectedTableElements.forEach((row) => dispatch(action({ "simulation-user-config-id": row.id as number })));
  };
  ...
  useEffect(() => {
    ...
    const rows = formatConfigurationRow(rowsServer, navigate);
    ...
    <Wrapper>
      <TableHeader>
        ...
        <AddConfigurationWrapper>
          <Button color="blue" size="small" href={`${pages.modifyConfigurationFile()}/`}>
            Add new simulation configuration
          </Button>
        </AddConfigurationWrapper>
      </TableHeader>
      <TableBody>
        ...
        {!!selectedTableElements.length && (
          <FooterActions>
            ...
            <Buttons>
              <Button color="blue" onClick={handleActionCustomViews(duplicateConfigurationFileServer)}>
                Duplicate selected simulation configuration
              </Button>
              <Button color="red" onClick={handleActionCustomViews(deleteConfigurationFileServer)}>
                Delete selected simulation configuration
              </Button>
            </Buttons>
          </FooterActions>
        )}
...
const headers: Header[] = [
  ...
  {
    text: "Default initial conditions file",
    align: "center",
    filterable: false,
    width: 20.634,
  },
  {
    text: "Override initial conditions file",
    align: "center",
    filterable: false,
    width: 18.412,
  },
...
export default withSimulation(ConfigurationFilesList);
